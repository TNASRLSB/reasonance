<script lang="ts">
  ...
  let { id = '', label = 'Agent', llm = '', nodeState = 'idle' as AgentState, selected = false, onselect }: {
    id?: string;
    label?: string;
    llm?: string;
...
>
  ...
  <div class="node-meta">
    ...
    <span class="node-state" style="color: {borderColor}" aria-hidden="true"><span class="state-icon" aria-hidden="true">{stateIcon}</span> {nodeState}</span>
  </div>
</button>
