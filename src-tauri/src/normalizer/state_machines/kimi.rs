...
impl StateMachine for KimiStateMachine {
    fn process(&mut self, event: AgentEvent) -> Vec<AgentEvent> {
        ...
        match event.event_type {
            ...
            // Text, Thinking, Error, Usage, Done — pass through unchanged
            _ => vec![event],
        }
    }
...
mod tests {
    ...
    fn test_thinking_passes_through() {
        let mut sm = KimiStateMachine::new();
        let event = AgentEvent::thinking("step 1: consider the input", "kimi");
        let result = sm.process(event);
        assert_eq!(result.len(), 1);
    ...
    fn test_reset_clears() {
        ...
        let start = AgentEvent::tool_use("list_files", "{}", "kimi");
        sm.process(start);
