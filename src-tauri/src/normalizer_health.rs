...
pub fn evaluate_test_case(test_case: &TestCase, events: &[AgentEvent]) -> TestCaseResult {
    ...
    for expected in &test_case.expected {
        ...
        let validation_passed = matching_events
            .iter()
            .any(|e| validate_event(e, &expected.validate));
...
fn validate_event(event: &AgentEvent, validation: &Validation) -> bool {
    match validation {
        ...
        Validation::ContentNotEmpty => match &event.content {
            EventContent::Text { value } => !value.is_empty(),
            EventContent::Code { source, .. } => !source.is_empty(),
            _ => true,
        },
        Validation::ContentMatches(pattern) => match &event.content {
...
mod tests {
    use super::*;
    use crate::agent_event::{AgentEvent, AgentEventMetadata, AgentEventType, EventContent};
    ...
    fn test_evaluate_basic_text_pass() {
        ...
        let events = vec![make_text_event("REASONANCE_TEST_OK"), make_done_event()];
    ...
    fn test_health_status_from_results() {
        ...
        assert!(matches!(
            health_status_from_results(&all_pass),
            HealthStatus::Healthy
        ));
        ...
        let some_fail = vec![
            ...
            TestCaseResult {
                name: "thinking".to_string(),
                passed: false,
                failure_reason: Some("missing".into()),
            },
        ];
        assert!(matches!(
            health_status_from_results(&some_fail),
            HealthStatus::Degraded { .. }
        ));

        let all_fail = vec![TestCaseResult {
            name: "basic_text".to_string(),
            passed: false,
            failure_reason: Some("no text".into()),
        }];
        assert!(matches!(
            health_status_from_results(&all_fail),
            HealthStatus::Broken { .. }
        ));
    }
}
