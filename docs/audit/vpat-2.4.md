...
|----------|-------------------|--------------------------|
| **1.1.1 Non-text Content** | Partially Supports | Most components provide `aria-label` on non-text controls. Terminal container lacks `role` and `aria-label`. DiffView accept/reject buttons lack explicit `aria-label`. |
...
| **1.3.1 Info and Relationships** | Partially Supports | Semantic landmarks, ARIA tree widget, tablists, dialog roles used throughout. MenuItem uses `role="menubar"` on individual items instead of parent bar (incorrect ARIA structure). TerminalManager uses `<span role="button">` instead of native `<button>`. |
| **1.3.2 Meaningful Sequence** | Supports | DOM order matches visual layout across all components. Minor concern with Toast stacking order. |
...
| **2.1.1 Keyboard** | Supports | Global shortcuts, focus traps in dialogs, searchable command palette. FileTree keyboard navigation added (tabindex, Enter/Space, arrows). MenuItem submenus keyboard-accessible (ArrowRight/Left). Toast dismiss reachable via keyboard (tabindex, focus pauses timer). |
| **2.1.2 No Keyboard Trap** | Supports | Dialogs implement focus trap with Escape exit. ResponsePanel now has a focus trap and Escape close. |
| **2.1.4 Character Key Shortcuts** | Supports | No single-character shortcuts without modifier keys. |
| **2.2.1 Timing Adjustable** | Partially Supports | Toast notifications auto-dismiss. No mechanism for keyboard users to pause or extend timers. |
...
| **4.1.1 Parsing** | Supports | Well-formed HTML throughout. Minor semantic issues (TerminalManager `<span role="button">`, MenuItem `role="menubar"` placement) but parseable. |
| **4.1.2 Name, Role, Value** | Supports | Strong ARIA coverage throughout. Terminal container has `role="tabpanel"` and `aria-label`. DiffBlock has `aria-expanded` and `aria-label`. ResponsePanel has `role="dialog"` and `aria-label`. MenuItem `role="menubar"` moved to parent container. |
...
| **1.4.11 Non-text Contrast** | Partially Supports | Most UI components meet 3:1. ResponsePanel close button lacks distinct visual boundary. StatusBar/Toast borders may blend in some themes. |
...
| **2.4.6 Headings and Labels** | Partially Supports | Descriptive headings and labels in most components. Terminal and ResponsePanel lack `aria-label`. TerminalManager add button "+" lacks label. |
| **2.4.7 Focus Visible** | Partially Supports | Focus rings on most interactive elements. Toast buttons unreachable. Terminal search buttons may lack visible focus ring at small sizes. |
...
All previously blocking issues have been resolved as of 2026-03-24. Remaining minor items:
...
3. **Toast auto-dismiss** — no user-configurable mechanism to extend the auto-dismiss timer (minor; focus pauses timer)
4. **TerminalManager "+" button** — add-tab button lacks descriptive `aria-label` beyond the "+" character

---

## Legal Disclaimer

This VPAT is provided for informational purposes and represents a self-assessment of accessibility conformance based on internal code review, component-level WCAG 2.1 matrix audit, and manual testing. It is not a certification of accessibility. Third-party testing is recommended before making formal conformance claims. This document reflects the state of Reasonance IDE version 0.11.0 as of 2026-03-24 following AA conformance fixes and may not reflect subsequent updates.
